...
for (const email of usersToSet) {
  ...
  const existing = db.prepare("SELECT id FROM mailbox_table WHERE email = ?").get(email);
  if (existing) {
    db.prepare("UPDATE mailbox_table SET password_hash = ?, plain_password = ? WHERE id = ?").run(hash, password, existing.id);
    console.log(`Updated user ${email} with password 12345678`);
  } else {
    db.prepare("INSERT INTO mailbox_table (email, password_hash, plain_password) VALUES (?, ?, ?)").run(email, hash, password);
    console.log(`Created user ${email} with password 12345678`);
  }
}

console.log("All configured users:", db.prepare("SELECT id, email, plain_password FROM mailbox_table").all());
