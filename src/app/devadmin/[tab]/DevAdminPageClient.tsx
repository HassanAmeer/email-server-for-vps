...
// Dev Admin constants
const DEV_TOKEN_KEY = "dev_admin_token";
const DEV_API_PREFIX = "/api/dev-admin";
...
export function DevAdminPageClient({ tabSegment }: DevAdminPageClientProps) {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        const token = localStorage.getItem(DEV_TOKEN_KEY);
        const res = await fetch(`${apiUrl}${DEV_API_PREFIX}/stats`, {
      ...
      <div className="flex bg-[#080810] text-gray-100 h-screen w-screen relative overflow-hidden font-sans">
        ...
        <div className="flex-grow flex flex-col h-full relative z-10 overflow-hidden bg-[#080810]">
          <header className="p-6 border-b border-white/[0.04] flex justify-between items-center bg-[#0A0A1A]/75">
            ...
            <div className="h-8 w-32 bg-violet-500/[0.05] rounded-full animate-pulse"></div>
          </header>
  ...
  // Sidebar navigation sections (ALL tabs always visible for devadmin)
  const coreNavItems = [
    {
      tab: "overview-tab",
      label: "Overview",
  ...
  return (
    <div className="flex bg-[#080810] text-gray-100 h-screen w-screen relative overflow-hidden font-sans">
      ...
      {/* Left Sidebar */}
      <aside
        className={`w-[280px] bg-[#0A0A1A]/95 backdrop-blur-xl border-r border-violet-500/[0.08] flex flex-col z-30 shrink-0 h-full transition-transform duration-300
      ...
      >
        ...
        <div className="p-6 border-b border-violet-500/[0.08] flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-violet-600 to-purple-400 flex items-center justify-center shadow-[0_0_15px_rgba(139,92,246,0.35)]">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor" className="w-5 h-5 text-white">
                <path strokeLinecap="round" strokeLinejoin="round" d="M17.25 6.75L22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3l-4.5 16.5" />
              </svg>
            </div>
            <div className="flex flex-col">
              <span className="font-extrabold text-sm tracking-widest uppercase font-mono bg-gradient-to-r from-violet-300 via-purple-200 to-slate-400 bg-clip-text text-transparent">
                Dev Admin
              </span>
              <span className="text-[9px] font-bold uppercase tracking-wider font-mono text-violet-400">
                Full Root Panel
              </span>
            </div>
          </div>
      ...
      <div className="flex-grow flex flex-col h-full relative z-10 overflow-y-auto bg-[#080810]">
        ...
        <header className="lg:hidden p-4 border-b border-violet-500/[0.08] flex items-center relative z-10 bg-[#0A0A1A]/75 backdrop-blur-md">
          ...
          <span className="ml-3 text-xs font-bold text-violet-400 font-mono uppercase tracking-widest">Dev Admin</span>
        </header>

        {/* Page Content */}
        <div className="p-8 flex-grow relative z-10">
          {activeTab === "overview-tab" && (
            <Overview apiUrl={apiUrl} stats={stats} apiPrefix={DEV_API_PREFIX} tokenKey={DEV_TOKEN_KEY} />
          )}
          {activeTab === "logs-tab" && (
            <LiveLogs apiUrl={apiUrl} systemMode={stats.liveModeActive ? "Live" : "Local"} apiPrefix={DEV_API_PREFIX} tokenKey={DEV_TOKEN_KEY} />
          )}
          {activeTab === "projects-tab" && (
            <ProjectsManager apiUrl={apiUrl} apiPrefix={DEV_API_PREFIX} tokenKey={DEV_TOKEN_KEY} />
          )}
          {activeTab === "domains-tab" && (
            <DomainsManager apiUrl={apiUrl} apiPrefix={DEV_API_PREFIX} tokenKey={DEV_TOKEN_KEY} />
          )}
          {activeTab === "primary-domain-tab" && (
            <PrimaryDomainManager apiUrl={apiUrl} apiPrefix={DEV_API_PREFIX} tokenKey={DEV_TOKEN_KEY} />
          )}
          {activeTab === "api-tab" && (
            <ApiSettingsManager apiUrl={apiUrl} apiPrefix={DEV_API_PREFIX} tokenKey={DEV_TOKEN_KEY} />
          )}
          {activeTab === "seeding-tab" && (
            <DataSeedingManager apiUrl={apiUrl} apiPrefix={DEV_API_PREFIX} tokenKey={DEV_TOKEN_KEY} />
          )}
          {activeTab === "credentials-tab" && (
            <CredentialsManager apiUrl={apiUrl} apiPrefix={DEV_API_PREFIX} tokenKey={DEV_TOKEN_KEY} />
          )}
